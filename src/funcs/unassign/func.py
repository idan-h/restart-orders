...
from library.functions import unassign_product
...
def handler(ctx: context, data: io.BytesIO = None):
    ...
    response_dict = {}
    try:
        user_id = parsed_url.query.split('=')[1]
        if not user_id:
        ...
        success = unassign_product(API_KEY, body["orderId"], body["subItemId"], body["subItemBoardId"], ORACLE_USER, ORACLE_PASSWORD, ORACLE_DSN)
        if success:
